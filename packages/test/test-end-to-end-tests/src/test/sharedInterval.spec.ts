...
function testIntervalCollection(intervalCollection: IntervalCollection<SequenceInterval>) {
    ...
    for (i = 0, result = iterator.next(); !result.done; i++, result = iterator.next()) {
        ...
        assert(interval === tempArray[i], "Mismatch in forward iteration with start position");
    }
    assert(i === tempArray.length, "Interval omitted from forward iteration with start position");
    ...
    for (i = 0, result = iterator.next(); !result.done; i++, result = iterator.next()) {
        const interval = result.value;
        assert(interval === tempArray[i], "Mismatch in backward iteration with start position");
    }
    assert(i === tempArray.length, "Interval omitted from backward iteration with start position");
    ...
    for (i = 0, result = iterator.next(); !result.done; i++, result = iterator.next()) {
        const interval = result.value;
        assert(interval === tempArray[i], "Mismatch in backward iteration with end position");
    }
    assert(i === tempArray.length, "Interval omitted from backward iteration with end position");
